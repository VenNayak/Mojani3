...
for (var i = 0; i < records.length; i++) {
		...
		console.log("LandRecord request body :" +JSON.stringify(landDetailHL));
		console.log("calling block chain code to insert assignee... #" + assigneeReq.owner.aadharNo); 
		needle('post','https://landrecord.mybluemix.net/api/AddAssignee', assigneeReq,function(err, resp,body){
             if(err){
				console.log("Error while inserting assignee to blockchain:" + assigneeReq.owner.aadharNo + " Err :" + err);
					 res.json({success : false, message : err+""});
			 }else{
			    console.log("Response body :" + JSON.stringify(resp.body));
				if(resp.body && resp.body.error && resp.body.error.statusCode == 500){
				  console.log("An error occurred while inserting the owner record to blockchain");
				  	 res.json({success : false, message : ""});
				}else{
			    console.log("Assignee inserted to blockchain " + JSON.stringify(body));
			 			needle('post','https://landrecord.mybluemix.net/api/AddLandRecord', landDetailHL, function(err,resp){
						if(err){
							console.log("Error while inserting landrecord to blockchain:" + err);
											 res.json({success : false, message : err+""});
						}else{
							console.log("Response body :" + JSON.stringify(resp.body));
							console.log("Inserting land record to blockchain: #" + landDetailHL.landrecord.pid);
							if(resp.body && resp.body.error && resp.body.error.statusCode == 500){
							 console.log("An error occurred while inserting the land record to blockchain");
							 				 res.json({success : false, message : err+""});
							 
							}else{
							console.log("Land Record inserted to blockchain :" + landDetailHL.landrecord.pid);					
			   ...
			   } //outer api call status check else
			}
       });
							
							break;
							} //check if mojani record is approved
			}
